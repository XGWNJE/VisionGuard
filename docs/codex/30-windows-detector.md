...
- 实时预览卡片区**最多 4 张、不分页**：`Views/CardGridPanel.cs`（自定义 `Panel`）与 `Services/CardLayoutPlanner.cs`（纯计算求解器）按面板自身可用像素求网格——1 张 = 1×1、2 张 = 1×2 或 2×1（取更宽的一边）、3–4 张 = 2×2；单元格宽高比夹紧在 **1:1.2 ~ 1.2:1**（长边不超过短边的 1.2 倍），超出时收窄格子并把空间留成间隔，宁可留白也不把卡片拉宽。`Views/UniformFramePresenter.cs` 让原始帧只在固定画面区内等比居中缩放，黑色背景承接留白；它不把极端来源的自然尺寸带回卡片测量，因此标题、状态和底部按钮不会被挤压或遮盖。卡片自身 UI 已压缩到最小：标题行 26、操作行 28、内边距 3、外边距 2、间距 4（`CardLayoutPlanner.CardChromeHeight` = 68，改卡片模板必须同步该常量），底部信息条由两行四段并为一行三段，省下的 35 DIP 全部给画面区。
- 推理输入与预览使用同一条等比规则：`ImagePreprocessor.Prepare()` 把任意来源居中缩放到方形模型输入，其余区域填纯黑，并把缩放与留白元数据交给 `YoloOutputParser`；解析器去除留白偏移后再映射回原始帧，完全落在黑边的检测框会被丢弃、跨边界的框会裁剪。因此 UI 检测框、报警截图和告警坐标始终保持原始帧坐标。选区长短边比严格超过 **2:1** 时，当前来源页以非阻断提示显示有效画面占比与“1:1 最佳”；2:1 / 1:2 边界不提示，用户仍可正常启动推理。
...
- 用户选择“完整退出”时，检测端先调用驻留的专用退出事件、取消登录自启，并等待驻留单实例互斥体消失；确认成功才关闭主体。若任一步失败，主体保持运行并显示原因，避免留下未知的后台状态；下次手动启动检测端会按正常启动路径恢复驻留与自启。
