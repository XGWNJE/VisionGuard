...
- `PrintWindow` 返回全黑画面时会产生“疑似黑屏”采集故障并显示在对应来源，不再只写调试日志。DirectML 在运行期发生推理异常时，多来源协调器会异步停止所有运行来源并保留故障原因，避免静默形成多路 CPU；捕获类故障仍限制在单一来源。FPS 只统计成功帧。
- WPF 四窗口人员 smoke 已通过独立 `WindowHandle` 捕获、逐路 `person`、每路 3 FPS、停止/重配隔离、移动缩放、遮挡、最小化故障隔离及恢复、关闭隔离、CPU 多路拒绝、DirectML 初始化回退和受控运行期故障全局停机；动态视频、其他 GPU、显卡驱动故障、持续运行、报警链和 UI 视觉仍需单独验收。
